...
async fn test_ankiconnect_connection() {
    let client =
        AnkiClient::new("http://localhost:8765".to_string()).expect("Failed to create client");
...
async fn test_get_decks() {
    ...
    assert!(
        !deck_list.is_empty(),
        "Deck list should not be empty (at least 'Default' should exist)"
    );
    assert!(
        deck_list.contains(&"Default".to_string()),
...
async fn test_create_and_add_note() {
    ...
    println!(
        "Note: You may want to manually delete the '{}' deck in Anki.",
        deck_name
    );
}
